...
chooseExpQ g n t bf (AppT ListT ty) = [| vectorOf ($(varE  n) `min` 10) $ resize ($(varE  n) `div` 10) arbitrary |]
...
simfreqStr :: Name -> Name -> Name ->  [ConView] -> ExpQ
simfreqStr g n t xs =
  [| frequency $ zip (getFreqs name) $(listE (makeArbs g n t xs)) |]
   where name = "sim"++(show t)

reqfreqStr :: Name -> Name -> Name ->  [ConView] -> ExpQ
reqfreqStr g n t xs =
  [| frequency $ zip (getFreqs name) $(listE (makeArbs g n t xs)) |]
   where name = "req"++(show t)
...
countConstructors  (TyConI (DataD _ _ _ TH211MBKIND constructors _ )) = length constructors
...
deriveArbitrary t = do
    inf <- reify t
    runIO $ print $ "('" ++ show t ++ "' , " ++ show (countConstructors inf) ++ ")"
    case inf of
              ...
              let ns  = map varT $ paramNames params
                  ...
                  gos g n = -- Fancy gos
                      ...
                      then
                            ...
                            then
                                reqfreqStr g n t fcs
                            else
                                if length fcs > 1
                                then
                                    [|  if $(varE n) <= 1
                                    then $(simfreqStr g n t fcs)
                                    else $(reqfreqStr g n t scons)|]
                                else
                                    [|  if $(varE n) <= 1
